...
class WordLoader:
    ...
    def __init__(self, filepath=None):
        # If no filepath provided, use wordlist.txt in same directory
        if filepath is None:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            filepath = os.path.join(script_dir, "wordlist.txt")
    ...
    def _load_words(self, filepath):
        try:
            with open(filepath, "r") as wordlist:
                return [line.strip() for line in wordlist if line.strip()] 
        except FileNotFoundError:
            messagebox.showerror("File Not Found", f"Could not find the word list file: {filepath}")
            exit(1)
...
class GUI(BaseApp):
    ...
    def __init__(self):
        super().__init__()
        self.word_loader = WordLoader()  # No argument needed now
        self.logic = LemonType(self.word_loader, self)
        self.build_gui()
